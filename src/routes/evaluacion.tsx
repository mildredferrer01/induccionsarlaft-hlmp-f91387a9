...
function ExamPage() {
  ...
  const handleStart = useCallback((u: CourseUser) => {
    setUser(u);
    saveUser(u);
    setStage("exam");
  }, []);
  ...
  const handleSubmit = useCallback(() => {
    ...
    const passed = score >= COURSE_META.passingScore;
    const computed: Result = { correct, total: QUESTIONS.length, score, passed };

    setResult(computed);
    saveResponse({ user, answers, score, passed, submittedAt: new Date().toISOString() });
    guardarEnSheets({
      nombre: user.name,
  ...
  if (stage === "result" && result) return <ResultView user={user} result={result} answers={answers} onRetry={reset} />;
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <section className="mx-auto max-w-xl px-6 py-12 md:py-16">
        <div className="relative overflow-hidden rounded-3xl border border-border bg-card" style={{ boxShadow: "var(--shadow-elegant)" }}>
          <div className="relative overflow-hidden px-8 py-10 text-primary-foreground" style={{ background: "var(--gradient-hero)" }}>
            ...
            <h1 className="relative mt-4 text-2xl md:text-3xl font-bold leading-tight">Antes de comenzar, identifícate</h1>
            <p className="relative mt-2 text-sm text-primary-foreground/80">Tus datos se registran junto con tus respuestas para emitir la constancia.</p>
          </div>
          <form onSubmit={submit} className="space-y-5 px-8 py-8">
            <div className="space-y-2">
              ...
              <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Ej. María Pérez Gómez" className="h-11" autoComplete="name" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="id" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Documento de identidad</Label>
              <Input id="id" value={idNumber} onChange={(e) => setIdNumber(e.target.value)} placeholder="Número de cédula" inputMode="numeric" className="h-11" />
            </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button type="submit" size="lg" className="flex-1">
                Comenzar evaluación <ArrowRight className="ml-1 h-4 w-4" />
              </Button>
            </div>
          </form>
        </div>
...
/* ---------- Exam ---------- */
function ExamSlide({
  current, setCurrent, answers, setAnswers, onSubmit,
}: {
  current: number;
  ...
  const next = () => { if (!isLast) { setCurrent(current + 1); window.scrollTo({ top: 0, behavior: "smooth" }); } };
  const prev = () => { if (!isFirst) { setCurrent(current - 1); window.scrollTo({ top: 0, behavior: "smooth" }); } };
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <section className="mx-auto max-w-2xl px-6 py-8 md:py-10">
        <div key={q.id} className="relative overflow-hidden rounded-3xl border border-border bg-card" style={{ boxShadow: "var(--shadow-elegant)" }}>
          <div className="px-7 py-6 text-primary-foreground" style={{ background: "var(--gradient-hero)" }}>
            <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-primary-foreground/80">Pregunta {current + 1} de {total}</p>
              ...
              return (
                <button key={oi} type="button"
                  ...
                  className={["group flex w-full items-center gap-4 rounded-xl border-2 p-4 text-left transition-all",
                    isSel ? "border-primary bg-primary/5" : "border-border hover:border-primary/40 hover:bg-secondary/60"].join(" ")}
                >
                  <span className={["flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-sm font-bold transition",
                    isSel ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground group-hover:bg-primary/10 group-hover:text-primary"].join(" ")}>
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-1.5">
          {QUESTIONS.map((qq, j) => (
            <button key={qq.id} aria-label={`Pregunta ${j + 1}`} onClick={() => setCurrent(j)}
              className={["h-2 rounded-full transition-all",
                j === current ? "w-7 bg-primary" : answers[qq.id] !== undefined ? "w-2 bg-primary/60" : "w-2 bg-border"].join(" ")}
            />
          ))}
        </div>
        <div className="mt-8 flex items-center justify-between gap-3">
          <Button variant="ghost" onClick={prev} disabled={isFirst}><ArrowLeft className="mr-2 h-4 w-4" /> Anterior</Button>
          {isLast ? (
            <Button size="lg" disabled={!allAnswered} onClick={onSubmit}>
              {allAnswered ? "Enviar evaluación" : `Faltan ${total - answeredCount}`}
            </Button>
          ) : (
            <Button size="lg" onClick={next} disabled={!hasAnswer}>Siguiente <ArrowRight className="ml-2 h-4 w-4" /></Button>
          )}
        </div>
      </section>
    </main>
...
/* ---------- Result ---------- */
function ResultView({ user, result, answers, onRetry }: {
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="mx-auto max-w-3xl px-6 py-12">
        <div className="overflow-hidden rounded-3xl border border-border bg-card" style={{ boxShadow: "var(--shadow-elegant)" }}>
          <div className="relative overflow-hidden px-8 py-12 text-center text-primary-foreground" style={{
            background: passed ? "var(--gradient-hero)" : "linear-gradient(135deg, oklch(0.40 0.18 25), oklch(0.55 0.22 30))",
          }}>
            <div className="absolute inset-0 opacity-30 pointer-events-none" style={{ background: "var(--gradient-mesh)" }} />
            ...
            <p className="relative mt-2 text-primary-foreground/85">
              {passed ? "Has completado satisfactoriamente la inducción SARLAFT."
          ...
          {user && (
            <div className="border-t border-border bg-secondary/40 px-8 py-6">
              <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Participante</p>
              ...
              <p className="text-sm text-muted-foreground">Fecha: {new Date().toLocaleDateString("es-CO", { dateStyle: "long" })}</p>
            </div>
          )}
                  ...
                  <li key={q.id} className="rounded-lg border border-border p-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="text-sm">
                        <p className="font-medium text-foreground">{idx + 1}. {q.prompt}</p>
                        <p className="mt-1 text-muted-foreground">Tu respuesta: {q.options[userAns]}</p>
                        {!ok && <p className="mt-0.5 text-success">Correcta: {q.options[q.correctIndex]}</p>}
                      </div>
                    </div>
                  </li>
                );
...
function Stat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="rounded-2xl border border-border bg-card p-5 text-center">
      ...
      <p className={["mt-1 text-3xl font-bold tabular-nums", highlight ? "text-success" : "text-foreground"].join(" ")}>{value}</p>
    </div>
  );
}
